...
export const MenuTab: React.FC<MenuTabProps> = ({
  ...
  const SideDishSelector: React.FC<{
    ...
    if (available.length === 0) {
      return (
        <p className="text-[10px] text-red-400 italic">No side dishes found</p>
      );
    }
    ...
    return (
      <div className="flex flex-col gap-1.5 mt-2 p-3 bg-neutral-50 rounded-xl border border-neutral-100 max-h-40 overflow-y-auto custom-scrollbar">
        <p className="text-[10px] font-bold text-neutral-400 mb-1 uppercase tracking-tight">
          Allowed Sides:
        </p>
        {available.map((item) => (
          <button
          ...
          >
            ...
            {!selected.includes(item.name) && (
              <Square className="w-3.5 h-3.5 text-neutral-200 block group-hover:hidden" />
            )}
            <span
              className={
                selected.includes(item.name)
  ...
  const handleApply = () => {
    ...
    if (
      ...
      !window.confirm(t("modals.overwrite_warning"))
    )
      return;
            ...
            <thead>
              <tr>
                {[
                  ...
                  t("menu.name"),
                  t("menu.price"),
                  t("menu.included_side") || "Included Side",
                  t("menu.status"),
                  t("cards.actions"),
                ].map((h) => (
                  <th
                    key={h}
                    className="p-5 font-serif italic text-xs uppercase tracking-widest text-neutral-400 border-b border-neutral-100"
                  >
                    {h}
                  </th>
                ))}
            ...
            <tbody className="divide-y divide-neutral-100">
                    ...
                    <div className="flex flex-col">
                      <button
                        ...
                        className={`w-10 h-10 flex items-center justify-center rounded-xl transition-all ${
                          item.hasIncludedSide
                            ...
                            : "bg-neutral-50 text-neutral-300 hover:bg-neutral-100 hover:text-neutral-500"
                        }`}
                      ...
                      {item.hasIncludedSide && (
                        <SideDishSelector
                          ...
                          available={editingMenu.filter((m) =>
                            isSideDishCategory(m.category),
                          )}
                          onChange={(names) =>
                            onUpdateItem(item.id, "sideChoices", names)
                          }
                        />
                      )}
                    </div>
              ...
              {editingMenu.length === 0 && (
                <tr>
                  <td
                    colSpan={5}
                    className="p-12 text-center text-neutral-400 italic text-sm"
                  >
                  ...
                  <>
                    {parsed.detectedDate && (
                      <div className="flex items-center gap-2 px-4 py-2 bg-amber-50 border border-amber-200 rounded-xl text-amber-800">
                        ...
                        <span className="text-sm font-medium">
                          {t("menu.detected_date") || "Detected date"}:{" "}
                          <strong>{parsed.detectedDate}</strong>
                        </span>
                      </div>
                    )}
                    ...
                    {parsed.items.length === 0 ? (
                      <p className="text-center text-sm text-neutral-400 italic py-8">
                        No items could be parsed. Check the format and try
                        again.
                      </p>
                    ) : (
                      <div className="max-h-72 overflow-y-auto custom-scrollbar rounded-2xl border border-neutral-200">
                        <table className="w-full text-sm">
                          <thead className="sticky top-0 bg-neutral-50 border-b border-neutral-100 z-10">
                            <tr>
                              <th className="p-3 text-left font-mono text-[10px] uppercase tracking-widest text-neutral-400 w-1/4">
                                {t("menu.category")}
                              </th>
                              <th className="p-3 text-left font-mono text-[10px] uppercase tracking-widest text-neutral-400 w-2/5">
                                {t("menu.name")}
                              </th>
                              <th className="p-3 text-left font-mono text-[10px] uppercase tracking-widest text-neutral-400">
                                {t("menu.price")}
                              </th>
                              <th className="p-3 text-center font-mono text-[10px] uppercase tracking-widest text-neutral-400">
                                {t("menu.side")}
                              </th>
                              <th className="p-3 text-right font-mono text-[10px] uppercase tracking-widest text-neutral-400">
                                {t("cards.actions")}
                              </th>
                            </tr>
                          </thead>
