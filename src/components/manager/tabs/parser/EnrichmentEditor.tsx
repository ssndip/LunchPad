...
export const EnrichmentEditor: React.FC<EnrichmentEditorProps> = ({
  rules,
  onChange,
  t,
}) => {
    ...
    <div className="space-y-6">
      <div className="bg-neutral-50 p-4 rounded-2xl border border-neutral-100 flex justify-between items-center">
        <div>
          <h4 className="text-sm font-black uppercase tracking-tighter text-neutral-900">
            {t("parser.enrichment_rules")}
          </h4>
          <p className="text-[10px] text-neutral-400">
            {t("parser.enrichment_desc")}
          </p>
        </div>
        <button
          onClick={addRule}
      ...
      <div className="space-y-4">
        {rules.map((rule) => (
          <div
            key={rule.id}
            className="p-6 bg-white border border-neutral-100 rounded-2xl shadow-sm group hover:border-neutral-200 transition-all"
          >
            <div className="flex flex-col md:flex-row gap-8 items-start">
              ...
              <div className="flex-1 space-y-4 w-full">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div>
                    <label className="text-[9px] font-black uppercase tracking-widest text-neutral-400 mb-1 block">
                      {t("parser.categories_comma")}
                    </label>
                    <input
                      type="text"
                  ...
                  <button
                    onClick={() =>
                      updateRule(rule.id, {
                        action: {
                          ...rule.action,
                          hasIncludedSide: !rule.action.hasIncludedSide,
                        },
                      })
                    }
