...
export const SectionDetectionEditor: React.FC<SectionDetectionEditorProps> = ({ sections, onChange }) => {
  const addSection = () => {
  ...
  const updateSection = (id: string, updates: Partial<SectionDetectionRule>) => {
  ...
  return (
    <div className="space-y-6">
      <div className="flex justify-between items-center bg-neutral-50 p-4 rounded-2xl border border-neutral-100">
        <div>
          <h4 className="text-sm font-black uppercase tracking-tighter text-neutral-900">Category Detection</h4>
          <p className="text-[10px] text-neutral-400">Define keywords that mark the start of a menu category.</p>
        </div>
        <button 
          onClick={addSection}
      ...
      <div className="grid grid-cols-1 gap-4">
        {sections.map((section, index) => (
          <div key={section.id} className="p-6 bg-white border border-neutral-100 rounded-2xl shadow-sm relative group overflow-hidden">
             {/* Number badge */}
             ...
             <div className="grid grid-cols-1 md:grid-cols-12 gap-6 mt-2">
                <div className="md:col-span-4">
                   <label className="text-[10px] font-black uppercase tracking-widest text-neutral-400 mb-1.5 block">Category Display Name</label>
                   <input 
                      type="text"
                      value={section.categoryName}
                      onChange={(e) => updateSection(section.id, { categoryName: e.target.value })}
                ...
                <div className="md:col-span-6">
                   <RuleField 
                      ...
                      onChange={(val) => updateSection(section.id, { pattern: val })}
                ...
                <div className="md:col-span-2 flex flex-col justify-end">
                   <div className="flex items-center gap-3 bg-neutral-50 p-3 rounded-xl border border-neutral-100 h-[46px]">
                      <div className="flex-1 flex items-center gap-2">
                         <Box className={`w-4 h-4 ${section.applyBoxFeeByDefault ? 'text-amber-500' : 'text-neutral-300'}`} />
                         <span className="text-[9px] font-black uppercase tracking-tighter text-neutral-500">Box Fee</span>
                      </div>
                      <button 
                        onClick={() => updateSection(section.id, { applyBoxFeeByDefault: !section.applyBoxFeeByDefault })}
                        className={`w-10 h-5 rounded-full transition-all relative ${section.applyBoxFeeByDefault ? 'bg-amber-500' : 'bg-neutral-200'}`}
                      >
                         <div className={`absolute top-1 w-3 h-3 bg-white rounded-full transition-all ${section.applyBoxFeeByDefault ? 'right-1' : 'left-1'}`} />
                      </button>
                   </div>
                </div>
             </div>
        ...
        {sections.length === 0 && (
          <div className="p-12 text-center border-2 border-dashed border-neutral-100 rounded-3xl">
             ...
             <p className="text-xs text-neutral-400 italic">No category rules defined. Use the button above to add one.</p>
          </div>
        )}
      </div>
    </div>
