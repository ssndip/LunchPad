...
export const SectionDetectionEditor: React.FC<SectionDetectionEditorProps> = ({ sections, onChange, t }) => {
  const addSection = () => {
  ...
  return (
    <div className="space-y-6">
      <div className="flex justify-between items-center bg-neutral-50 p-4 rounded-2xl border border-neutral-100">
        <div>
          <h4 className="text-sm font-black uppercase tracking-tighter text-neutral-900">{t('parser.section_detection')}</h4>
          <p className="text-[10px] text-neutral-400">{t('parser.section_detection_desc')}</p>
        </div>
        <button 
          onClick={addSection}
      ...
      <div className="grid grid-cols-1 gap-4">
          ...
          <div key={section.id} className="p-6 bg-white border border-neutral-100 rounded-2xl shadow-sm relative group overflow-hidden">
             ...
             <div className="grid grid-cols-1 md:grid-cols-12 gap-6 mt-2">
                <div className="md:col-span-4">
                   <label className="text-[10px] font-black uppercase tracking-widest text-neutral-400 mb-1.5 block">{t('parser.cat_display_name')}</label>
                   <input 
                      type="text"
                ...
                <div className="md:col-span-2 flex flex-col justify-end">
                   <div className="flex items-center gap-3 bg-neutral-50 p-3 rounded-xl border border-neutral-100 h-[46px]">
                      <div className="flex-1 flex items-center gap-2">
                         ...
                         <span className="text-[9px] font-black uppercase tracking-tighter text-neutral-500">{t('parser.auto_box')}</span>
                      </div>
                      <button 
        ...
        {sections.length === 0 && (
          <div className="p-12 text-center border-2 border-dashed border-neutral-100 rounded-3xl">
             ...
             <p className="text-xs text-neutral-400 italic">{t('parser.no_section_rules')}</p>
          </div>
        )}
      </div>
    </div>
