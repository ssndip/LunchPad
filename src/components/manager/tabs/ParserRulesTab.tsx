...
export const ParserRulesTab: React.FC<ParserRulesTabProps> = ({ confirm }) => {
  ...
  const handleActivateProfile = (profile: ParserProfile) => {
    confirm({
      ...
      onConfirm: () => {
        ...
        setTimeout(
          () => handleParse(profile.presets, profile.settings.activePresetId),
          100,
        );
      },
    });
    ...
    const exportData = {
      ...
      _ai_instructions: {
        ...
        schema: {
          ...
          preprocessRules:
            "Array of { find: string, replace: string, isRegex: boolean }",
        ...
        instruction:
          "To handle a new menu format, add regex rules to 'preprocessRules' that normalize the text into a standard bulleted list format: '- Item Name 1.23€'. Use itemCategoryOverrides to fix misclassified items.",
      },
    };
  ...
  // ── Parse ──
  const handleParse = (
    forcedPresets?: FormatPreset[] | React.MouseEvent,
    ...
    const currentList =
      forcedPresets && Array.isArray(forcedPresets) ? forcedPresets : presets;
    const currentActiveId =
      forcedPresetId !== undefined && !isEvent
        ? forcedPresetId
        : activePresetId;
  ...
  const applyClassify = () => {
    ...
    if (action === "category") {
      ...
      addPresetRule({
        find: line.trim(),
        replace: inputValue.trim(),
        isRegex: false,
      });
    } else if (action === "item") {
      // Save a preset rule: prepend "- " to this line pattern
      addPresetRule({
        find: `^(${line.trim().replace(/[.*+?^${}()|[\]\\]/g, "\\$&")})$`,
        replace: "- $1",
        isRegex: true,
      });
    ...
    setParseResult((prev) =>
      prev
        ? {
            ...prev,
            unmatchedLines: prev.unmatchedLines.filter(
              (_, i) => i !== classify.index,
            ),
          }
        : null,
    );
  ...
  const addPresetRuleSync = (rule: {
    ...
    if (targetId) {
      ...
      if (idx >= 0) {
        updated[idx] = {
          ...updated[idx],
          preprocessRules: [...updated[idx].preprocessRules, rule],
        };
      }
    } else {
      const now = new Date().toISOString();
  ...
  const addRemoveItemRule = (itemName: string) => {
    const { updatedPresets, activeId } = addPresetRuleSync({
      find: itemName,
      replace: "",
      isRegex: false,
    });
        ...
        <div className="p-6">
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-6">
            <div className="flex items-center gap-3">
              ...
              <div>
                <h3 className="text-sm font-black text-neutral-900 uppercase tracking-tight">
                  {t("parser.profiles_title")}
                </h3>
                <p className="text-[10px] text-neutral-400 font-medium">
                  {t("parser.snapshot_tip")}
                </p>
              </div>
            </div>
            <div className="flex items-center gap-2">
              ...
              >
                <div className="flex items-start justify-between mb-4">
                  <div className="min-w-0">
                    <p className="text-xs font-black text-neutral-900 truncate pr-8">
                      {profile.name}
                    </p>
                ...
                <div className="flex items-center gap-3 mb-4">
                  <div className="flex flex-col">
                    ...
                    <span className="text-xs font-black text-neutral-700">
                      {(profile.presets || []).reduce(
                        (acc, p) => acc + (p.preprocessRules?.length || 0),
                        0,
                      )}
                    </span>
                  </div>
                  ...
                  <div className="flex flex-col">
                    <span className="text-[10px] font-bold text-neutral-400 uppercase tracking-tighter">
                      {t("parser.item_remaps")}
                    </span>
                    <span className="text-xs font-black text-neutral-700">
                      {(profile.presets || []).reduce(
                        (acc, p) =>
                          acc +
                          Object.keys(p.itemCategoryOverrides || {}).length,
                        0,
                      )}
                    </span>
                  </div>
      ...
      <div className="bg-white rounded-3xl border border-neutral-100 shadow-sm overflow-hidden">
          ...
          <div className="flex items-center gap-3">
            ...
            <div className="text-left">
              <h2 className="text-sm font-black uppercase tracking-widest text-neutral-900">
                {t("parser.category_settings")}
              </h2>
              <p className="text-[10px] text-neutral-400 mt-0.5">
                {t("parser.category_settings_desc")}
              </p>
            </div>
          </div>
          <div className="flex items-center gap-3">
        ...
        <div className="px-6 py-6 grid grid-cols-1 md:grid-cols-2 gap-3">
          ...
          <div className="col-span-full grid grid-cols-[1fr_auto_auto] gap-4 px-3 pb-1 border-b border-neutral-100">
            ...
            <span className="text-[10px] font-black uppercase tracking-widest text-neutral-400 w-24 text-center flex items-center gap-1 justify-center">
      ...
      <div className="bg-white rounded-3xl border border-neutral-100 shadow-sm overflow-hidden">
        <div className="p-6 border-b border-neutral-50 bg-neutral-50/10">
          <div className="flex items-center gap-3 mb-1">
            ...
            <div>
              <h2 className="text-sm font-black uppercase tracking-widest text-neutral-900">
                {t("parser.live_preview")}
              </h2>
              <p className="text-[10px] text-neutral-400 mt-0.5">
                {t("parser.live_preview_desc")}
              </p>
            </div>
          </div>
        </div>
            ...
            <div className="flex-1 flex flex-col gap-3 min-w-0">
              ...
              <div className="flex gap-3">
                ...
                >
                  ...
                  {isNormalizing
                    ? t("parser.normalizing")
                    : `✨ ${t("parser.auto_format")}`}
                </button>
              </div>
            </div>
                ...
                <>
                  ...
                  <div className="flex flex-wrap gap-2 shrink-0">
                    ...
                    <div className="flex items-center gap-1.5 px-3 py-1.5 bg-green-50 border border-green-200 rounded-xl text-[10px] font-bold text-green-800">
                      ...
                      {parseResult.items.length} {t("parser.items")}
                    </div>
                    {parseResult.unmatchedLines.length > 0 && (
                      <div className="flex items-center gap-1.5 px-3 py-1.5 bg-red-50 border border-red-200 rounded-xl text-[10px] font-bold text-red-700">
                        ...
                        {parseResult.unmatchedLines.length}{" "}
                        {t("parser.unmatched")}
                      </div>
                    )}
                  </div>
                  ...
                  {Object.entries(groupedItems).map(([cat, items]) => {
                    ...
                    return (
                      ...
                      >
                        <div
                          className={`px-4 py-2 flex items-center gap-2 ${colors.bg}`}
                        >
                          <span
                        ...
                        {items.map((item, i) => (
                          <div
                            key={i}
                            className="flex flex-col border-t border-neutral-50 px-4 py-2 hover:bg-neutral-50 transition-colors group"
                          >
                            <div className="flex items-center gap-2 text-xs">
                              <span className="flex-1 font-bold text-neutral-900 truncate">
                                {item.name}
                              </span>
                              {item.hasIncludedSide && (
                                <Layers
                                  className="w-3 h-3 text-indigo-500 shrink-0"
                                  title={t("parser.side_dish")}
                                />
                              )}
                              {item.tags?.includes("autobox") && (
                                <Package
                                  className="w-3 h-3 text-amber-500 shrink-0"
                                  title={t("parser.auto_box")}
                                />
                              )}
                    ...
                    <div className="mt-4 p-4 bg-indigo-50 border border-indigo-100 rounded-2xl flex items-center justify-between">
                      <div>
                        <h4 className="text-xs font-bold text-indigo-900">
                          {t("parser.preset_rules_applied")}
                        </h4>
                        <p className="text-[10px] text-indigo-600">
                          {t("parser.editing_active")}
                        </p>
                      </div>
                      <button
          ...
          >
            <div className="p-6 border-b border-red-50 bg-red-50/40">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <h2 className="text-sm font-black uppercase tracking-widest text-neutral-900">
                    {t("parser.unmatched_lines")}{" "}
                    <span className="font-mono text-red-500">
                      ({parseResult.unmatchedLines.length})
                    </span>
                  </h2>
                  <p className="text-[10px] text-neutral-400 mt-0.5">
                    {t("parser.classify_desc")}
                  </p>
                </div>
              </div>
            </div>
              ...
              {parseResult.unmatchedLines.map((line, idx) => (
                <div key={idx}>
                  <div className="flex items-center gap-3 p-3 bg-red-50 border border-red-100 rounded-2xl">
                    <code className="flex-1 text-xs font-mono text-red-800 truncate">
                      {line}
                    </code>
                    <button
                      onClick={() =>
        ...
        <div className="p-6 space-y-4">
          ...
          {presets.map((preset) => (
            ...
            >
              <div className="flex items-center justify-between mb-2">
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-[10px] text-neutral-400">
                    {preset.preprocessRules.length} rule
                    {preset.preprocessRules.length !== 1 ? "s" : ""}
                  </span>
                </div>
                <div className="flex gap-2">
                  <button
                  ...
                  <button
                    onClick={() =>
                      setActivePresetId(
                        activePresetId === preset.id ? null : preset.id,
                      )
                    }
                  ...
                  <button
                    onClick={() =>
                      confirm({
                        title: t("parser.delete_preset"),
                        message: `${t("modals.delete_warning")} "${preset.name}"?`,
                        isDestructive: true,
              ...
              {preset.itemCategoryOverrides &&
                Object.keys(preset.itemCategoryOverrides).length > 0 && (
                  <div className="space-y-1 mt-3">
                    <p className="text-[9px] font-bold text-neutral-400 uppercase">
                      {t("parser.item_remaps")}
                    </p>
                    {Object.entries(preset.itemCategoryOverrides).map(
                      ([itemName, targetCat], i) => (
                        ...
                        >
                          ...
                          <span className="text-neutral-400">→</span>
                          <code className="px-2 py-0.5 bg-indigo-50 border border-indigo-100 text-indigo-700 rounded text-[9px] uppercase tracking-widest font-black">
                            {targetCat}
                          </code>
                        </div>
                      ),
              ...
              {preset.itemNameOverrides &&
                Object.keys(preset.itemNameOverrides).length > 0 && (
                  <div className="space-y-1 mt-3">
                    <p className="text-[9px] font-bold text-neutral-400 uppercase">
                      {t("parser.item_renames")}
                    </p>
                    {Object.entries(preset.itemNameOverrides).map(
                      ([originalName, newName], i) => (
                        ...
                        >
                          ...
                          <span className="text-neutral-400">→</span>
                          <code className="px-2 py-0.5 bg-emerald-50 border border-emerald-100 text-emerald-700 rounded text-xs font-bold">
                            {newName}
                          </code>
                        </div>
                      ),
              ...
              {manualRulePreset === preset.id ? (
                  ...
                  <button
                    onClick={() => {
                      ...
                      setPresets((prev) => {
                        ...
                        if (idx >= 0) {
                          updated[idx].preprocessRules.push({
                            find: manualRuleFind,
                            replace: manualRuleReplace,
                            isRegex: false,
                          });
                        }
                ...
                <div className="mt-3">
                  <button
                    onClick={() => {
                      setManualRulePreset(preset.id);
                      setManualRuleFind("");
                      setManualRuleReplace("");
                    }}
