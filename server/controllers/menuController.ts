...
export const fetchMenuBackups = (req: Request, res: Response, next: NextFunction) => {
  try {
    // ⚡ Bolt: Calculate itemCount inside SQLite via JSON1 extension to prevent N+1 queries,
    // prevent fetching large JSON strings into Node, and avoid blocking V8 memory with JSON.parse()
