...
## 2026-04-24 - AI API Key Public Exposure Leak
**Vulnerability:** The application was leaking the `aiApiKey` from the database settings to any public client via the unauthenticated `GET /api/init` bootstrapping endpoint and the WebSocket `INITIAL_STATE` broadcast, as they blindly returned the raw `settings.aiApiKey`.
**Learning:** Returning raw, sensitive backend configuration variables to public bootstrap endpoints guarantees that administrative secrets will be leaked to the public internet unless explicitly masked or filtered out.
**Prevention:** When returning sensitive configuration values via public endpoints, always mask the value (e.g., using `********`). Furthermore, ensure the corresponding update endpoints (e.g., `updateSettings`) explicitly check for and ignore this masked placeholder to avoid accidentally overwriting the valid secret in the database when the client resubmits its configuration payload.
