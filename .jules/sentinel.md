...
## 2026-04-24 - IDOR Brute-force Risk on Hardware Integrated Endpoints
**Vulnerability:** Unauthenticated `GET /api/cards/:rfid/profile` serving the public Kiosk was vulnerable to IDOR brute-force.
**Learning:** Endpoints that use physical possession (RFID) as their access mechanism cannot use standard authentication. They are uniquely vulnerable to enumeration/brute-forcing.
**Prevention:** Implement endpoint-specific rate-limiting (e.g., `express-rate-limit`) for hardware-integrated public endpoints. Explicitly exempt local/whitelisted IPs to prevent locking out the legitimate hardware clients while blocking remote attackers.
