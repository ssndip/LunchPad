...
## 2026-04-24 - Prevent Secret Exposure in REST and WS Responses
**Vulnerability:** The application was exposing the `aiApiKey` in plaintext to any client via HTTP responses (like `GET /api/init`, `fetchSettings`, and `updateSettings`) and WebSocket broadcasts (like `INITIAL_STATE` and `SETTINGS_UPDATE`).
**Learning:** Sending the entire `settings` object (or an unchecked subset) to public endpoints or broadcasts guarantees that newly added sensitive administrative settings will be leaked.
**Prevention:** Always mask sensitive configuration values (e.g., using '********') before returning them in API endpoints or broadcasts. Additionally, ensure update endpoints explicitly check for and ignore the masked placeholder to avoid accidentally overwriting the valid secret in the database.
