...
## 2025-02-28 - Custom Toggles Accessibility
**Learning:** Custom switch toggles built with `button` elements (like the ones in `SettingsTab.tsx` and `CardsTab.tsx`) often lack proper ARIA switch roles and keyboard focus visibility, falling back to basic button semantics and invisible keyboard states. `aria-pressed` was incorrectly used instead of `aria-checked` alongside `role="switch"`.
**Action:** Always ensure custom toggles use `role="switch"`, `aria-checked={value}` (not `aria-pressed`), and explicit keyboard focus visibility (e.g., `focus-visible:ring-2 focus-visible:ring-offset-2`).
