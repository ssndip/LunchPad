...
## 2024-05-19 - Descriptive ARIA labels on dynamic lists
**Learning:** Icon-only buttons mapped from dynamic lists must have descriptive aria-labels (e.g., `Remove ${item.name}`) so screen reader users know which specific item the button affects, rather than just reading "Remove".
**Action:** Always append dynamic content context (like item names) to aria-labels on icon buttons in mapped lists.
