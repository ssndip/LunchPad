...
## 2024-11-21 - Accessible Icon-Only Buttons
**Learning:** Found several icon-only utility buttons (e.g., in KioskOrderPanel, MenuTab) that had visually intuitive icons but lacked `aria-label` attributes. Without these, screen readers announce "button" or "unlabelled button," creating a frustrating experience for vision-impaired users. Furthermore, relying only on `title` attributes may not provide sufficient context to assistive technologies.
**Action:** Ensure all icon-only buttons include localized `aria-label`s for screen reader support alongside `title` attributes for desktop hover states. I applied this fix to `KioskOrderPanel`, `KioskView`, and `MenuTab` components.
