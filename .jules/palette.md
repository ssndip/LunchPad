...
**Learning:** When generating `aria-label`s for interactive elements inside a mapped list (like a shopping cart), static labels like `"Remove"` lack context when read by a screen reader.
**Action:** Always interpolate contextual data into the ARIA label when applicable (e.g., ``aria-label={`${t("modals.remove")} ${item.name}`}``) so users know exactly which item they are interacting with.
