...
## 2024-05-23 - Localized ARIA labels on utility buttons
**Learning:** Even well-intentioned accessibility features like `title` and `aria-label` can fall short if they are hardcoded and not localized. I found several utility buttons in `KioskItemList.tsx`, `KioskOrderPanel.tsx`, and `KioskView.tsx` that lacked `aria-label` entirely, despite having `title` attributes using the translation function `t()`. This makes them completely invisible to screen readers while remaining accessible via hover for mouse users.
**Action:** Always verify that utility button attributes include both `title` AND `aria-label` and ensure both are wrapped in `t()` using translation keys.
